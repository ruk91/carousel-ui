...
import carouselStyles from "../styles/Carousel.scss"
import arrowsStyles from "../styles/Arrows.scss"
import FontAwesomeIcon from "@fortawesome/react-fontawesome"
import faChevronLeft from "@fortawesome/fontawesome-free-solid/faChevronLeft"
import faChevronRight from "@fortawesome/fontawesome-free-solid/faChevronRight"
import { DotsHomeSlider } from "../index"
export default class Carousel extends Component {
    static propTypes = {
        ...
        bottomArrows: PropTypes.bool,
        dotsWithArrows: PropTypes.bool,
    ...
    getSelectedValue = (value) => {
        const maxValue = this.getChildren().length
        if (value >= maxValue) {
    ...
    renderCarouselItems = () => {
        ...
        return (
            <div className={carouselStyles.BrainhubCarousel__trackContainer}>
                <ul
                    className={[carouselStyles.BrainhubCarousel__track, transitionEnabled ? carouselStyles["BrainhubCarousel__track--transition"] : "", draggable ? carouselStyles["BrainhubCarousel__track--draggable"] : ""].join(" ")}
                    style={trackStyles}
    ...
    calculateButtonValue = (val) => {
        const homeCarousel  = this.getProp("slideData")
        const numberOfSlides = homeCarousel.length
        return val >= 0
            ? val % numberOfSlides
            : val + numberOfSlides * Math.ceil(Math.abs(val / numberOfSlides))
    }
    getCurrentHotelName = () => {
        const homeCarousel  = this.getProp("slideData")
        return homeCarousel.find((el, i) => i === this.calculateButtonValue(this.getCurrentValue())).caption
    }
    getCurrentHotelLink = () => {
        const homeCarousel  = this.getProp("slideData")
        return homeCarousel.find((el, i) => i === this.calculateButtonValue(this.getCurrentValue())).link
    }
    renderDots() {
        if (this.getProp("dots")) {
            return (
                <Dots value={this.getCurrentValue()} onChange={this.changeSlide} number={this.getChildren().length} />
            )
        }
        return null
    }
    ...
    renderDotsWithArrows = () => {
        // if (this.getProp("dotsWithArrows")) {
        return (
            <div className={arrowsStyles.arrowContainer}>
                <div onClick={this.prevSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.right} icon={faChevronLeft} />
                </div>
                <Dots value={this.getSelectedValue(this.getCurrentValue())} onChange={this.changeSlide} number={this.getChildren().length} />
                <div onClick={this.nextSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.right} icon={faChevronRight} />
                </div>
            </div>
        )
        // }
    ...
    renderDotsWithArrowsHome = () => {
        ...
        return (
            <div className={arrowsStyles.arrowContainer}>
                ...
                <DotsHomeSlider value={this.getSelectedValue(this.getCurrentValue())} onChange={this.changeSlide} number={this.getChildren().length} />
                {/*<Dots value={this.getSelectedValue(this.getCurrentValue())} onChange={this.changeSlide} number={this.getChildren().length} />*/}
                <div onClick={this.nextSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.right} icon={faChevronRight} />
                </div>
    ...
    renderArrowsWithoutDots = () => {
        return (
            <div className={arrowsStyles.arrowContainer}>
                <div onClick={this.prevSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.arrowLeft} icon={faChevronLeft} />
                </div>
                <div onClick={this.nextSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.arrowRight} icon={faChevronRight} />
                </div>
            </div>
        )
    }
    renderArrowsForOverly = () => {
        return (
            <div className={arrowsStyles.arrowContainerForOverly}>
                <div onClick={this.prevSlide} className={arrowsStyles.noSelectWithBackground}>
                    <FontAwesomeIcon className={arrowsStyles.overlyArrowLeft} icon={faChevronLeft} />
                </div>
                <div onClick={this.nextSlide} className={arrowsStyles.noSelectWithBackground}>
                    <FontAwesomeIcon className={arrowsStyles.overlyArrowRight} icon={faChevronRight} />
                </div>
            </div>
        )
    }
    ...
    renderThumbnails = () => {
        return (
            <div className={arrowsStyles.arrowContainer}>
                ...
                <div className={arrowsStyles.scrolling_wrapper}>
                    <Dots thumbnails={this.props.slides} value={this.getSelectedValue(this.getCurrentValue())} onChange={this.changeSlide} number={this.getChildren().length} />
                </div>
                <div onClick={this.nextSlide} className={arrowsStyles.noSelect}>
                    <FontAwesomeIcon className={arrowsStyles.right} icon={faChevronRight} />
                </div>
    ...
    render() {
        return (
            ...
            >
                ...
                {this.getProp("homeCarousel") ?
                    <div style={{position: "relative"}}>
                        {this.renderCarouselItems()}
                        <div className={carouselStyles.arrowsCarouselContainer}>
                            <div>{this.renderDotsWithArrowsHome()}</div>
                        </div>
                        <div className={carouselStyles.withoutDots}>
                            <div>{this.renderArrowsWithoutDots()}</div>
                        </div>
                        <div className={carouselStyles.hotelNameMain}>
                            <div className={carouselStyles.nameContainer}>
                                <a
                                    className={carouselStyles.viewHotelNewCarousel}
                                    href={this.getCurrentHotelLink()}
                                    style={{ float: "right", zIndex: "11", fontFamily: "Raleway-Bold", fontWeight: "bold" }}
                                >
                                    {this.getCurrentHotelName()}
                                    <FontAwesomeIcon icon={faChevronRight} style={{ marginLeft: 5, fontSize: "0.9rem", fontWeight: "normal" }} />
                                </a>
                            </div>
                        </div>
                    </div>
                    : (this.getProp("overlyArrows") ?
                        <div className={carouselStyles.overlyArrowsMain}>
                            {this.renderCarouselItems()}
                            <div className={carouselStyles.overlyArrows}>
                                {this.renderArrowsForOverly()}
                            </div>
                        </div>
                        : this.renderCarouselItems())
                }
                ...
                {this.renderDots()}
                {this.getProp("dotsWithArrows") && this.renderDotsWithArrows()}
                {this.getProp("thumbnails") && this.renderThumbnails()}
            </div>
        )
    }
}
